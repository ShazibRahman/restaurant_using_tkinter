...
def finalpay():
    ...
    messagebox.showinfo(
        'succes', 'your transaction is succesful please wait for server response you will be redirected in a few seconds')
    for i in file.read():
        msg += i
    file.close()
    b = False
    ...
    if b:
        messagebox.showinfo(
            'mail', 'your order detail has been sent to your email ')
    payroot.destroy()


def cashpay():
    messagebox.showinfo(
        'success', 'Please hand over money to the cashier and check your email')
...
def debtpayment():
    ...
    debtlabelframe = tk.LabelFrame(
        debt, text="DEBIT PAYEMENT METHOD", font=('calibri', 10, 'italic'))
    ...
    cancel = Button(debtlabelframe, width=10,
                    text="CANCEL", command=debt.destroy)
...
def onlinepayment():
    ...
    onllabelframe = tk.LabelFrame(
        onlinepay, text="NET BANKING", font=('calibri', 20, 'italic'), fg='blue')
    ...
    cancel = Button(onllabelframe, width=10, text="CANCEL",
                    command=onlinepay.destroy)
...
def pay():
    ...
    proceed = Button(paylabelframe, width=10,
                     text="PROCEED", command=pay_check)
    proceed.grid(row=3, column=0, padx=10, ipady=3, ipadx=3)
    cancel = Button(paylabelframe, width=10, text="CANCEL",
