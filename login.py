...
def loginf():
    ...
    fr=Frame(login)
    fr.pack(side=TOP)
    labinfo=tk.Label(fr, text='Restaurant login page', font=('calibri',12,'italic'),justify='center')
    labinfo.pack(padx=10, pady=10)
    lbframe=tk.LabelFrame(login, text='Login',font=('calibri',20,'italic'),fg='green')
...
def signup():
    ...
    labinfo.pack()
    lbframe=tk.LabelFrame(sign, text='Sign up',font=('calibri',20,'italic'),fg='green')
    lbframe.pack(padx=10 , pady=10 , ipadx=10 , ipady=10)
...
def login_():
    ...
    for i,l in b:
        if i==u.get().strip() and l==v.get().strip():
            flag=1
    if flag==1:
        messagebox.showinfo('succes',f'your are logged in as {u.get().strip()}')
        user=u.get().strip()
        login.destroy()
        ...
        k+=1
        messagebox.showwarning('fail','try again User id/password is incorrect')
...
def sign_():
    ...
    for i in arr:
        if i.get() =='' or i.get() =='select' or i.get() =='null':
            ...
            flag= True
    if flag:
        cursor.execute(f'insert into signup values("{w.get().strip()}","{x.get().strip()}","{y1.get().strip()}","{y2.get().strip()}","{y3.get().strip()}","{y4.get().strip()}", 0) ')
        mydb.commit()
        sign.destroy()
        loginf()
    else:
