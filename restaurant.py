...
def payement():
    ...
    if not payment_tot:
        a = messagebox.askyesno(
            'payement', 'totaling has not been done for the session do you want to totaling first ?')
        if a:
            total()
            if tot != 0.0:
                payement()
        ...
        cursor.execute(
            f'insert into payments values({order_no},"{usernam}","{tot}")')
        mydb1.commit()
        p.pay()
...
biryani_price = {'Null': 0, 'veg biryani': 100,
                 'chicken biryani': 200, 'mutton biryani': 250}
noodles_price = {'Null': 0, 'veg noodles': 50,
                 'egg noodles': 80, 'chicken noodles': 100}
french_price = {'Null': 0, 'small bowl fries': 40,
                'medium bowl fries': 70, 'large bowl fries': 100}
fried_price = {'Null': 0, 'veg fried rice': 70,
               'egg fried rice': 90, 'chicken fried rice': 110}
...
def total():
    ...
    cost_ = round(drink_c+biryani_c+dosa_c+fried_c +
                  noodles_c+idli_c+french_c, 3)
    ...
    if totaling and not already_payement:
        a = messagebox.askyesno(
            'totaling', 'totaling has already been done for this session. proceed to pay ?')
        if a:
            payement()
    elif totaling and already_payement:
        a = messagebox.askyesno(
            'payment', 'payment has already been done fot this session . Do you want to place a new order ?')
        if a:
            resetf()
        ...
        if cost_ >= 1000:
            messagebox.showinfo(
                'huraah', 'you got 10% off as you ordered for more than 1000 RS')
            tot = round(.9*tot, 2)
        ac.set(str(ac_))
        totaling = True
        payment_tot = True
        ...
        cursor.execute(
            f'insert into orders (name ,cost , gst , total) values ("{usernam}",{cost_},{tax}, {tot})')
        cursor.execute('select max(order_no) from orders')
        order_no = cursor.fetchone()[0]

        order_v.set(order_no)
        cursor.execute(
            f'insert into dosa values ({order_no},"{dosapopvar.get().strip()}",{dosa.get().strip()})')
        mydb1.commit()
        cursor.execute(
            f'insert into idli values ({order_no},"{idlipopvar.get()}",{idli.get()});')
        mydb1.commit()
        cursor.execute(
            f'insert into french values ({order_no},"{frenchpopvar.get()}",{french.get()});')
        mydb1.commit()
        cursor.execute(
            f'insert into biryani values ({order_no},"{biryanipopvar.get()}",{biryani.get()});')
        mydb1.commit()
        cursor.execute(
            f'insert into fried values ({order_no},"{friedpopvar.get()}",{fried.get()});')
        mydb1.commit()
        cursor.execute(
            f'insert into drink values ({order_no},"{drinkpopvar.get()}","{drinkvar.get()}");')
        mydb1.commit()
        cursor.execute(
            f'insert into noodles values ({order_no},"{noodlespopvar.get()}",{noodles.get()});')
        mydb1.commit()
        ...
        for i, j in [(dosapopvar, dosa), (idlipopvar, idli), (frenchpopvar, french), (biryanipopvar, biryani), (friedpopvar, fried), (drinkpopvar, drinkvar), (noodlespopvar, noodles)]:
            ...
            if i.get() == 'Null' or j.get() == '0':
                ...
                if i.get() != 'NUll' and j.get() != 'Null':
                    file.write(
                        f'{drinkvar.get()} {drinkpopvar.get().strip().upper()}\n\n')
            else:
                file.write(f'{j.get()} plate/es {i.get().strip().upper()}\n\n')
        if cost_ >= 1000:
            file.write(
                'hurrah you got 10% off as you ordered above 1000 RS,,Thank you\n\n')
        ...
        messagebox.showwarning(
            'please', 'please order something first before pressing the total button')
...
def bill():
    ...
    lableq = tk.Label(bill_, text="Orders", font=(
    ...
    if flag == 0:
        ...
        lb = tk.Label(bill_, text='  YOU ORDERED NOTHING:',
                      font=('calibri', 20, 'bold'), fg='red')
...
def price():
    ...
    proce = tk.LabelFrame(pricetk, text='Menu', font=(
        'calibri', 20, 'italic'), fg='green')
    proce.pack(padx=15, pady=15, ipadx=15, ipady=15)

    lb00 = tk.Label(proce, font=('aria', 10, 'bold'),
                    text='Dosa ', fg='black', bd=10)
    lb00.grid(pady=10, row=0, column=0)
    lb01 = tk.Label(proce, font=('aria', 10, 'italic'),
                    text='Masala Dosa RS. 50 ', fg='black')
    lb01.grid(pady=10, row=0, column=1)
    lb01 = tk.Label(proce, font=('aria', 10, 'italic'),
                    text='plane Dosa RS. 30 ', fg='black', bd=10)
    lb01.grid(pady=10, row=1, column=1)

    lb10 = tk.Label(proce, font=('aria', 10, 'bold'),
                    text='French Fries ', fg='black', bd=10)
    lb10.grid(pady=10, row=2, column=0)
    lb11 = tk.Label(proce, font=('aria', 10, 'italic'),
                    text='Small bowl 40 RS ', fg='black', bd=10)
    lb11.grid(pady=10, row=2, column=1)
    lb11 = tk.Label(proce, font=('aria', 10, 'italic'),
                    text='Medium  bowl 70 RS ', fg='black', bd=10)
    lb11.grid(pady=10, row=3, column=1)
    lb11 = tk.Label(proce, font=('aria', 10, 'italic'),
                    text='Large  bowl 100 RS ', fg='black', bd=10)
    lb11.grid(pady=10, row=4, column=1)

    lb20 = tk.Label(proce, font=('aria', 10, 'bold'),
                    text='Idli ', fg='black', bd=10)
    lb20.grid(pady=10, row=5, column=0)
    lb21 = tk.Label(proce, font=('aria', 10, 'italic'),
                    text='Idli Sambhar 40 RS ', fg='black', bd=10)
    lb21.grid(pady=10, row=5, column=1)
    lb21 = tk.Label(proce, font=('aria', 10, 'italic'),
                    text='Idli Chutney 50 RS ', fg='black', bd=10)
    lb21.grid(pady=10, row=6, column=1)

    lb30 = tk.Label(proce, font=('aria', 10, 'bold'),
                    text='Biryani ', fg='black', bd=10)
    lb30.grid(pady=10, row=7, column=0)
    lb31 = tk.Label(proce, font=('aria', 10, 'italic'),
                    text='Veg. Biryani 100 RS ', fg='black', bd=10)
    lb31.grid(pady=10, row=7, column=1)
    lb31 = tk.Label(proce, font=('aria', 10, 'italic'),
                    text='Chicken Biryani 200 RS ', fg='black', bd=10)
    lb31.grid(pady=10, row=8, column=1)
    lb31 = tk.Label(proce, font=('aria', 10, 'italic'),
                    text='Mutton Biryani 250 RS ', fg='black', bd=10)
    lb31.grid(pady=10, row=9, column=1)

    lb40 = tk.Label(proce, font=('aria', 10, 'bold'),
                    text='Fried Rice ', fg='black', bd=10)
    lb40.grid(pady=10, row=0, column=2)
    lb41 = tk.Label(proce, font=('aria', 10, 'italic'),
                    text='Veg Fried Rice 70 ', fg='black', bd=10)
    lb41.grid(pady=10, row=0, column=3)
    lb41 = tk.Label(proce, font=('aria', 10, 'italic'),
                    text='Egg Fried Rice  RS 90', fg='black', bd=10)
    lb41.grid(pady=10, row=1, column=3)
    lb41 = tk.Label(proce, font=('aria', 10, 'italic'),
                    text='chicken Fried Rice RS 110 ', fg='black', bd=10)
    lb41.grid(pady=10, row=2, column=3)

    lb50 = tk.Label(proce, font=('aria', 10, 'bold'),
                    text='DRINKS ', fg='black', bd=10)
    lb50.grid(pady=10, row=3, column=2)
    lb51 = tk.Label(proce, font=('aria', 10, 'italic'),
                    text='200 ml : 20 RS  ', fg='black', bd=10)
    lb51.grid(pady=10, row=3, column=3)
    lb61 = tk.Label(proce, font=('aria', 10, 'italic'),
                    text='1 Ltr : 56 RS  ', fg='black', bd=10)
    lb61.grid(pady=10, row=4, column=3)
    lb71 = tk.Label(proce, font=('aria', 10, 'italic'),
                    text='2 Ltr : 106 RS  ', fg='black', bd=10)
    lb71.grid(pady=10, row=5, column=3)

    lb40 = tk.Label(proce, font=('aria', 10, 'bold'),
                    text='Noodles ', fg='black', bd=10)
    lb40.grid(pady=10, row=6, column=2)
    lb41 = tk.Label(proce, font=('aria', 10, 'italic'),
                    text='Veg Noodles 50 RS ', fg='black', bd=10)
    lb41.grid(pady=10, row=6, column=3)
    lb41 = tk.Label(proce, font=('aria', 10, 'italic'),
                    text='Egg Noodles 80 RS ', fg='black', bd=10)
    lb41.grid(pady=10, row=7, column=3)
    lb41 = tk.Label(proce, font=('aria', 10, 'italic'),
                    text='chicken Noodles 100 RS ', fg='black', bd=10)
...
def admin(f1, infoimg, user):
    ...
    if admin == 1:
        orders = Button(f1, width=14, text="KITCHEN MGMT.",
                        command=kitchen_management, image=infoimg, compound=LEFT)
        orders.grid(pady=12, row=9, column=4, padx=10, ipady=5, ipadx=5)
        set_label()
        resetf()
    else:
        orders = Button(f1, width=14, text="ORDERS",
                        command=bill, image=infoimg, compound=LEFT)
        orders.grid(pady=12, row=9, column=4, padx=10, ipady=5, ipadx=5)
        set_label()
        resetf()
...
def main(user):
    ...
    messagebox.showinfo('Restaurant Management System',
                        'This project is designed and coded by Md Shazib Rahman ')
    ...
    localtime = time.asctime()
    # -----------------INFO TOP------------
    info = tk.Label(Tops, font=('aria', 28, 'bold'),
                    text="    KHAO PIYO RESTAURANT          ", fg="green", pady=10)
    log = tk.Label(Tops, image=logo)
    log.grid(row=0, column=0, ipady=0, ipadx=0, padx=0, pady=0)
    info.grid(row=0, column=1)
    info = tk.Label(Tops, font=('aria', 15, 'italic'),
                    text=localtime, fg="steel blue", bd=10)
   ...
   # labels entries and optionmenu
    ...
    Cheese_burger_label = tk.Label(
        f1, font=('aria', 10, 'bold'), text="Fried Rice")
    ...
    Service_Charge_label = tk.Label(
        f1, font=('aria', 10, 'bold'), text="AC Charge")
   ...
   # ---buttons-----
    ...
    Price_b = Button(f1, width=14, text="MENU", command=price,
                     image=menuimg, compound=LEFT)
    Price_b.grid(pady=20, row=9, column=0, padx=10, ipady=5, ipadx=5)

    Total = Button(f1, width=14, text="TOTAL", image=totalimg,
                   command=total, compound=LEFT)
    Total.grid(pady=10, row=9, column=1, padx=5, ipady=5)

    paybt = Button(f1, width=14, text="PAY", command=payement,
                   image=payimg, compound=LEFT)
    paybt.grid(pady=20, row=9, column=2, padx=10, ipady=5, ipadx=5)

    reset = Button(f1, width=14, text="RESET", command=resetf,
                   image=resetimg, compound=LEFT)
